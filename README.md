...
5. **Automated Generation**: These SBOMs are generated using [mikebom](https://github.com/kusari-sandbox/mikebom) with deps.dev enrichment for license and dependency resolution. The generation process runs weekly and supports **all ecosystems** that mikebom can detect (Go, Rust, npm, Python, Java/Maven, Ruby, and more).
...
- Generates SPDX 2.3-compliant SBOM files using [mikebom](https://github.com/kusari-sandbox/mikebom) (v0.1.0-alpha.9)
...
2. Each matrix job downloads mikebom and generates SBOMs using `mikebom sbom scan --format spdx-2.3-json`
...
- mikebom (auto-downloaded by the script, or install manually)

> **Note:** Go is no longer required for SBOM generation. mikebom is a precompiled Rust binary.
...
| `MIKEBOM_VERSION` | mikebom release version to use (default: v0.1.0-alpha.9) |
...
- Creator: `Tool: mikebom-0.1.0-alpha.9`
...
### mikebom Installation

mikebom is a precompiled binary — no build step required:

```bash
# The local script auto-downloads mikebom if not found in PATH
./util/generate-sbom-local.sh

# Or install manually
curl -sL https://github.com/kusari-sandbox/mikebom/releases/download/v0.1.0-alpha.9/mikebom-v0.1.0-alpha.9-x86_64-unknown-linux-gnu.tar.gz | tar xz
sudo cp mikebom-v0.1.0-alpha.9-x86_64-unknown-linux-gnu/mikebom /usr/local/bin/
```
...
deps.dev and ClearlyDefined enrichment runs inline. If network is slow, scans may take longer (2-5 minutes per repo is normal). Use `--offline` flag with mikebom to skip enrichment for faster but less complete SBOMs.
